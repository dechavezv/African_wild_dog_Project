"""
Usage:
python Sliding_window_HKA_Diversity.py chr_all_filtered.vcf.gz 1000000 10000 chr01 > outfile.txt
...
def computedPolymor(AF_all,sites_passing):
        SNPS_values=[]
        #print(AF_all)
        #print(len(AF_all))
	...
	Polymor=((numpy.sum(SNPS_values)/sites_passing)*1.1428571428571428)
        print('%s\t%d\t%d\t%f\t%d' % (chromo,window_start,window_end,Polymor,sites_passing))
...
def fetch_and_calc(chromo,start_pos,end_pos):
        ...
        computedPolymor(AF_all,sites_passing)
