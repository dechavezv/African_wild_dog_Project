"""
Usage:
python Sliding_window_HKA_DIVERGENCE.py chr_all_filtered.vcf.gz 1000000 10000 chr01 > outfile.txt
...
def computedPolymor(DS_all,sites_passing):
        SNPS_values=[]
        print(DS_all)
        #print(len(AF_all))
        for i in range(len(DS_all)):
 		if DS_all[i] == '1/1':
                	SNPS_values.append(float(1))
                elif DS_all[i] == '0/1':
                	SNPS_values.append(float(0.5))
	Diver=(numpy.sum(SNPS_values)/sites_passing)
        print('%s\t%d\t%d\t%f\t%d' % (chromo,window_start,window_end,Diver,sites_passing))
...
def fetch_and_calc(chromo,start_pos,end_pos):
        ...
        computedPolymor(DS_all,sites_passing)
